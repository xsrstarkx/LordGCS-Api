...
loghandler = {
    ...
    notkata: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de kata'
    },
    nottext: {
        status: false,
        creator: `${creator}`,
    ...
    nottext2: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de text2'
    },
    notnabi: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de nabi'
    },
    nottext3: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de text3'
    },
    nottheme: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de theme'
    },
    notusername: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de username'
    },
    notvalue: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros de value'
    },
    invalidKey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidlink: {
        status: false,
        creator: `${creator}`,
        message: 'erro, talvez seu link seja inválido.'
    },
    invalidkata: {
        status: false,
        creator: `${creator}`,
        message: 'erro, talvez a palavra não esteja no fogo.'
    },
    error: {
        status: false,
        creator: `${creator}`,
...
const listkey = ["LordGCS", "APIKEY", "gcs", "GCS", "lord", "Lord"];
...
router.get('/download/fb', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "parâmetro de url de entrada"})
    ...
    TikTokScraper.getUserProfileInfo(username)
        ...
        .catch(e => {
             res.json({
                 status : false,
                 creator : `${creator}`,
                 message : "erro, talvez seu nome de usuário seja inválido"
             })
         })
   } else {
res.json(loghandler.invalidKey)
...
router.get('/stalk/npm', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!query) return res.json({ status : false, creator : `${creator}`, message : "parâmetros de consulta de entrada"})
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (!Apikey) return res.json(loghandler.notparam)
		if (listkey.includes(Apikey)){
		if (!type) return res.json({status: false, creator, code: 404, message: 'parâmetro de tipo de entrada, tipo disponível : base64 , base32'})
		if (type == 'base64' && encode){
				...
				res.json({
					status:false,
					creator: `${creator}`,
					message: "adicionar parâmetros encode/decode"
				})
			} else {
				res.json(loghandler.error)
			}
...
router.get('/info/cuaca', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(!kota) return res.json({status: false, code: 406, message: 'insira os parâmetros da cidade'})
  if(listkey.includes(apikey)) {
    Cuaca(kota)
    .then((data) => {
      res.json(data)
...
router.get('/muslim/hadits', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!kitab) return res.json({ status : false, creator : `${creator}`, message : "parâmetros do livro de entrada"})
    if (!nomor) return res.json({ status : false, creator : `${creator}`, message : "número do parâmetro de entrada"})
...
router.get('/muslim/quran', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
    if (!surah) return res.json({ status : false, creator : `${creator}`, message : "insira os parâmetros de parágrafo"})
    if (!ayat) return res.json({ status : false, creator : `${creator}`, message : "parâmetros de entrada de parágrafo"})
...
router.get('/muslim/jadwalshalat', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!kota) return res.json({ status : false, creator : `${creator}`, message : "parâmetros da cidade de entrada"})
...
router.get('/info/wikipedia', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!search) return res.json({ status : false, creator : `${creator}`, message : "insira os parâmetros de pesquisa"})
...
router.get('/fakedata', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!country) return res.json({ status : false, creator : `${creator}`, message : "parâmetros do país de entrada"})
...
router.get('/hilih', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!kata) return res.json({ status : false, creator : `${creator}`, message : "parâmetro de palavra de entrada"})
...
router.get('/music/chordlagu', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!lagu) return res.json({ status : false, creator : `${creator}`, message : "entrada de parâmetro kata"})
...
router.get('/info/kbbi', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
        if(!kata) return res.json({ status : false, creator : `${creator}`, message : "insira os parâmetros de kata"})
...
router.get('/info/kodepos', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
	if(!kota) return res.json({ status : false, creator : `${creator}`, message : "insira os parâmetros de kota"})
...
router.get('/maker/ttp', async (req, res, next) => {
  ...
  Apikey = req.query.apikey;
  if (!req.query.text) return res.json({ status: 404, error: 'insira o parâmetro de texto'})
  if(!Apikey) return res.json(loghandler.notparam)
  if(listkey.includes(Apikey)) {
  random = new Date
